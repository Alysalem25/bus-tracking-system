...
const ProfilePage = () => {
  ...
  const { user: authUser, isLoading: authLoading, checkAuth } = useAuthStore()
  ...
  React.useEffect(() => {
    // Ensure auth is checked and populate the form when authUser is available
    if (!authUser) {
      // trigger auth check (no-op if already done)
      checkAuth().catch(() => {})
      return
    }
    ...
    setFormData({
      ...
      // auth user stores phone as `number` in the auth store
  ...
  // Show loading while the auth store is checking or no user yet
